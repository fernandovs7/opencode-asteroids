...
// ── Estado del juego ──────────────────────────────────────────────────────────
let ship, bullets, asteroids, particles, speedPowerUp, tripleShotPowerUp;
  ...
  for (const b of bullets) {
    for (const a of asteroids) {
      if (!a.dead && !b.dead && dist(b, a) < a.radius) {
        ...
        if (!speedPowerUp && !tripleShotPowerUp && Math.random() < 0.2) {
          if (Math.random() < 0.5)
            speedPowerUp = new SpeedPowerUp(a.x, a.y);
          else
...
function drawHUD() {
  ...
  if (ship.tripleShotTimer > 0) {
    ...
    ctx.fillText(`TRIPLE SHOT ${ship.tripleShotTimer.toFixed(1)}s`, W / 2, y);
  }
  ctx.fillStyle = '#fff';
